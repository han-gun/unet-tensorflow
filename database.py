...
class Database:
    ...
    def get_database(self, idx, mode='train'):
        label = np.zeros([len(idx), self.patch_y_size, self.patch_x_size, self.patch_ch_size])
        ...
        for i, idx_ in enumerate(idx):
            ...
            # __label = self.__label[iy:iy+self.patch_y_size, ix:ix+self.patch_x_size, :, idx[i]]
            # __input = self.__input[iy:iy+self.patch_y_size, ix:ix+self.patch_x_size, :, idx[i]]
    ...
    def _patch2image(self, src, type='count'):
        ...
        if nker[2] == 0:
            wgt_smx = np.ones((ncrop[0], ncrop[1], npatch[2] - 2 * nker[2]), 'float32')
        else:
